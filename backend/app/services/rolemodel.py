...
**피치 표준편차는 비교에 쓰지 않는다.**
같은 연사(김경일)의 강연 두 편이 전체 범위의 최소·최대를 동시에 차지했다.
같은 사람이 범위를 다 덮는 지표는 그날의 상태를 재는 것이지 발표 실력이 아니다.
(2026-08-12 측정: 37.9Hz / 47.9Hz, 전체 범위 37.9~47.9)
"""
import logging
...
def compare(user_summary: dict[str, Any] | None,
    ...
    return {
        ...
        "metrics": metrics,
        "excluded": {
            # 왜 뺐는지 남겨둔다. 나중에 "피치도 넣지"라는 말이 또 나온다.
            "pitch_std": "같은 연사의 두 강연이 전체 범위의 최소·최대를 차지해 개인 식별력이 없음",
        },
    }

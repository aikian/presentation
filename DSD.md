...
본 DSD의 설계 기준은 실시간 발표 흐름을 방해하지 않는 것과 발표 종료 후 재현 가능한 근거 기반 피드백을 생성하는 것에 둔다. 따라서 발표 중에는 손 제스처 제어처럼 즉시성이 필요한 기능만 최소 범위로 실행하고, 시선·자세·제스처 정밀 분석과 AI 코칭은 발표 종료 후 처리한다.

| 기준 | 적용 방식 |
|------|-----------|
| 발표 중 지연 최소화 | 발표 중 실시간 작업은 Hand Landmarker 기반 GestureWorker로 제한한다. |
| 개인정보 보호 | 원본 영상과 FrameData는 가능한 클라이언트에서 처리하고, 서버에는 집계값과 필요한 캡처 메타데이터만 전달한다. |
| 보고서 근거성 | 모든 코칭 항목은 `SessionSummary`, `issueEvents`, `captureId` 중 하나 이상의 근거와 연결한다. |
| 구현 가능성 | 1차 프로토타입에서는 PC 웹, 1인 발표, 웹캠 기반 비언어 분석 범위에 집중한다. |
| 확장성 | 음성 분석, 다중 사용자, 관리자 기능은 핵심 구조를 해치지 않도록 별도 확장 항목으로 둔다. |
...
그림 1은 1차 프로토타입 기준 전체 시스템 구성도이다. 실제 제출 형식에 따라 동일 구조를 draw.io 또는 Lucidchart 그림으로 변환할 수 있다.
...
| 결정 항목 | 결정 내용 | 근거 |
|-----------|-----------|------|
| 실시간 분석 범위 | 발표 중에는 GestureWorker만 실행 | Face/Pose까지 실시간 실행하면 브라우저 부하가 커지고 발표 화면 반응성이 떨어질 수 있다. |
| 정밀 분석 시점 | 발표 종료 후 녹화 Blob URL을 VideoAnalyzer가 처리 | 전체 발표 맥락을 기준으로 문제 구간을 선별하고, 분석 fps를 조절할 수 있다. |
| 서버 역할 | FastAPI는 세션 관리, 저장소 연동, Gemini 호출, PDF 생성에 집중 | Railway 서버에서 영상 추론을 수행하지 않아 서버 CPU 비용과 네트워크 지연을 줄인다. |
| 저장소 정책 | 캡처 이미지와 PDF는 Supabase Storage 비공개 버킷에 저장 | 보고서 근거 이미지를 관리하되 공개 URL 노출을 피한다. |
| AI 호출 방식 | 발표 종료 후 선별 캡처 최대 5개와 집계 JSON으로 1회 호출 | API 비용과 응답 지연을 줄이고, PDF 코칭 섹션의 길이를 제어한다. |

### 2.5 보안 및 실패 경계

시스템 경계는 원본 영상, 분석 집계, 보고서 산출물의 책임을 분리하는 방식으로 둔다. 원본 영상은 분석과 캡처 생성에만 사용하고, 보고서 생성 후 삭제 대상으로 관리한다. Supabase Storage에 남는 데이터는 캡처 이미지와 PDF 보고서이며, 접근은 인증된 사용자와 제한 시간 Signed URL을 기준으로 제어한다.

| 상황 | 처리 기준 |
|------|-----------|
| 웹캠 권한 거부 | 발표 시작을 막고 권한 재요청 또는 설정 안내를 표시한다. |
| Worker 초기화 실패 | 분석 불가 상태로 표시하고 세션 생성 전 단계에서 중단한다. |
| VideoAnalyzer 처리 실패 | 부분 분석 결과가 있으면 저장하되, 실패 원인을 보고서 메타데이터에 남긴다. |
| Gemini 호출 실패 | 최대 2회 재시도 후 규칙 기반 fallback 코칭으로 대체한다. |
| PDF 생성 실패 | 보고서 상태를 failed로 저장하고 재생성 요청이 가능하도록 한다. |
...
정면 응시 기준은 yaw ±15도, pitch ±10도를 초기 임계값으로 사용한다. 제스처 제어는 오작동을 줄이기 위해 동일 판정 10프레임 유지와 이벤트 발생 후 1초 쿨다운을 적용하며, 실제 웹캠 테스트 결과에 따라 confidence threshold와 함께 보정한다.
...
영상 분석 로직은 메인 UI 스레드와 분리되어 실행되므로, Worker와 React 메인 스레드 사이의 메시지 타입을 명확히 분리한다.

| Worker | 입력 메시지 | 주요 필드 | 출력 메시지 | 설명 |
|--------|-------------|-----------|-------------|------|
| GestureWorker | `INIT` | modelUrl, runningMode, confidence | `READY` | 발표 시작 전 Hand Landmarker를 초기화한다. |
| GestureWorker | `FRAME` | timestampMs, videoFrame 또는 imageBitmap | `GESTURE_EVENT` | 유지 조건을 만족한 `next`, `prev`, `pointer` 이벤트만 반환한다. |
| GestureWorker | `DISPOSE` | sessionId | `DISPOSED` | 발표 종료 시 모델과 프레임 참조를 해제한다. |
| VideoAnalyzer | `INIT` | faceModelUrl, handModelUrl, poseModelUrl, sampleFps | `READY` | 발표 후 분석용 3개 모델을 초기화한다. |
| VideoAnalyzer | `ANALYZE_FRAME` | timestampMs, videoFrame 또는 imageBitmap | `FRAME_RESULT` | 동일 timestamp 기준의 `FrameData`를 반환한다. |
| VideoAnalyzer | `FINALIZE` | sessionId | `SUMMARY_READY` | 누적 FrameData를 집계하여 `SessionSummary`를 반환한다. |
| 공통 | 모든 메시지 처리 중 예외 발생 | code, message, timestampMs | `ERROR` | 모델 로딩, 추론 실패, 필수 landmark 누락 등 예외를 전달한다. |
...
`SessionSummary`는 점수화 모듈의 `ScoreResult`, AI 코칭 모듈의 `CoachingRequest`, PDF 보고서 모듈의 입력으로 공통 사용될 수 있도록 평균 지표, 이벤트 타임라인, 캡처 식별자를 함께 포함한다.

`issueEvents`는 캡처 모듈과 AI 코칭 모듈이 같은 문제 구간을 참조할 수 있도록 다음 기준으로 분류한다.

| category | 대표 조건 | severity 산정 기준 | 후속 사용처 |
|----------|-----------|-------------------|-------------|
| gaze | 정면 응시 이탈, 시선 좌우 편향, 장시간 하향 시선 | 기준 각도 초과 크기와 지속 시간의 조합 | 캡처 트리거, 시선 코칭, 시선 점수 |
| posture | 어깨 기울기, 상체 흔들림, 상체 앞쏠림 | 기준 자세 대비 편차와 지속 시간의 조합 | 캡처 트리거, 자세 코칭, 자세 점수 |
| gesture | 손 움직임 과다, 제스처 부족, 얼굴 가림 후보 | 손 속도, 양손 대칭성, 제스처 빈도 조합 | 제스처 코칭, 제스처 점수 |
| time | 특정 슬라이드 체류 시간 과다 또는 부족 | 목표 시간 대비 오차율 | 시간 코칭, 시간 관리 점수 |
...
4. 자세 분석
   ...
   - 상체 앞쏠림은 발표 시작 직후 기준 자세에서 측정한 어깨-골반 거리 비율과 현재 비율의 차이로 추정한다.
   - confidence가 낮거나 필수 landmark가 누락된 프레임은 결측 프레임으로 표시하고 평균 계산에서 제외한다.
   - 시선 이탈, 자세 불균형, 제스처 과다 사용이 지정 지속 조건을 넘으면 `issueEvents`에 timestamp, category, reason, severity를 기록하여 캡처와 AI 코칭 입력으로 연결한다.

5. 품질 보정 및 종료 처리
   - 발표 시작 직후 2~3초 구간을 기준 자세 보정 구간으로 사용하여 어깨-골반 거리, 평균 상체 중심, 기본 EAR 값을 저장한다.
   - 모델 confidence가 기준값보다 낮은 프레임은 `valid=false`로 처리하고, 연속 결측 프레임이 길어지면 분석 품질 경고 이벤트를 추가한다.
   - `VideoAnalyzer`는 분석 진행률을 주기적으로 메인 스레드에 전달하여 결과 화면에서 처리 중 상태를 표시할 수 있도록 한다.
   - 분석 종료 후에는 `FrameData` 전체 대신 집계된 `SessionSummary`와 선별된 issue/capture 참조만 서버 저장 대상으로 전달한다.
...
Gemini 요청은 보고서 생성 API 내부 작업으로 실행되며, 다음 운영 파라미터를 기본값으로 둔다.

| 항목 | 기본값 | 설명 |
|------|--------|------|
| maxCoachingItems | 5 | PDF 보고서에서 읽기 쉬운 분량을 유지하기 위한 최대 코칭 항목 수 |
| maxImages | 5 | 시각 근거가 필요한 상위 문제 구간만 Gemini 입력에 포함 |
| retryCount | 2 | 일시적 API 오류 또는 네트워크 오류에 대한 재시도 횟수 |
| timeoutSec | 30 | 응답 지연이 길어질 경우 fallback 처리를 시작하는 기준 |
| language | ko-KR | 코칭 문장은 한국어로 생성 |
| tone | 개선 행동 중심 | 비난 표현을 피하고 다음 연습에서 바로 실행할 행동으로 제시 |

코칭 결과는 다음 품질 기준을 만족해야 한다.

| 기준 | 설명 |
|------|------|
| 근거성 | `issueEvents`, `SessionSummary`, 캡처 이미지 중 하나 이상의 근거를 포함한다. |
| 실행 가능성 | "자세를 개선하세요"처럼 추상적인 문장이 아니라 다음 연습에서 수행할 행동을 제시한다. |
| 중복 방지 | 같은 문제를 여러 문장으로 반복하지 않고 category별 핵심 이슈를 우선한다. |
| 안전성 | 건강 진단, 심리 상태 단정, 외모 평가처럼 발표 코칭 범위를 벗어난 표현을 금지한다. |
| 보고서 적합성 | PDF 한 항목에 들어갈 수 있도록 문제, 근거, 코칭, 개선 행동을 짧게 분리한다. |
...
`captureUrl`은 Gemini가 생성하지 않고, 백엔드가 `captureId`를 기준으로 저장소의 캡처 이미지 URL을 매핑하여 추가한다. Gemini 응답은 다음 JSON 배열 형태를 요구한다.
...
| 구분 | 규칙 |
|------|------|
| 역할 | 발표자의 비언어 습관을 분석하는 발표 코치로 응답한다. |
| 입력 해석 | 수치 데이터와 캡처 이미지를 함께 보되, 이미지에서 확실하지 않은 내용은 단정하지 않는다. |
| 표현 방식 | 문제 지적보다 개선 행동을 앞세우고, 발표자가 바로 따라 할 수 있는 문장으로 작성한다. |
| 출력 제한 | JSON Schema에 없는 필드나 markdown 문법을 응답에 포함하지 않는다. |
| 우선순위 | severity가 높은 항목을 우선하되 gaze, posture, gesture, time이 가능하면 균형 있게 포함되도록 한다. |

4. 응답 처리
   - Gemini 응답 문자열에서 JSON 배열을 파싱한다.
   - 필수 필드(`category`, `captureId`, `issue`, `evidence`, `coaching`, `improvement`, `severity`)가 없으면 해당 항목을 폐기하거나 fallback 항목으로 대체한다.
   - 파싱된 결과는 `captureId` 기준으로 `captureUrl`을 보강한 뒤 severity 기준으로 정렬하고 PDF 보고서 생성 모듈에 전달한다.
...
규칙 기반 fallback은 Gemini 응답을 대체하는 최소 품질 보장 장치이며, category별 대표 문구는 다음과 같이 구성한다.

| category | fallback 기준 | 기본 코칭 문구 |
|----------|---------------|----------------|
| gaze | `frontGazeRatio`가 기준보다 낮음 | 핵심 문장을 말할 때 카메라 방향을 2초 이상 유지하여 청중과의 연결감을 높인다. |
| posture | 어깨 기울기 또는 상체 흔들림이 기준 초과 | 슬라이드 전환 직후 양발을 고르게 딛고 어깨 높이를 맞춘 뒤 다음 문장을 시작한다. |
| gesture | 손 움직임 속도 과다 또는 제스처 부족 | 문단 전환 지점에서만 손동작을 사용하고, 설명 중에는 손 위치를 가슴 아래로 안정화한다. |
| time | 슬라이드 체류 시간이 목표 대비 크게 벗어남 | 각 슬라이드 첫 문장 전에 목표 체류 시간을 확인하고 핵심 문장 2~3개로 설명을 제한한다. |

보고서 저장 시에는 `modelName`, `promptVersion`, `fallbackUsed`, `selectedCaptureCount`, `createdAt`을 메타데이터로 함께 남겨 이후 코칭 품질을 비교할 수 있도록 한다.

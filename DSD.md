...
발표 중  영상은 MediaRecorder 기반으로 녹화되며, 발표 종료 후 VideoAnalyzer의 입력 데이터로 사용된다.

업로드된 슬라이드 파일은 발표 세션과 연결되어 Supabase Storage에 저장되며, 발표 중 슬라이드 표시와 보고서 생성 과정에서 공통으로 사용된다.

본 모듈은 발표 녹화 및 발표 후 영상 분석이 안정적으로 수행될 수 있도록 브라우저 권한 상태와 입력 장치 상태를 사전에 점검하며, 설정 완료 후 발표 세션을 생성한다.

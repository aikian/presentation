...
발표 중 영상은 MediaRecorder를 통해 클라이언트에서 로컬로 녹화되며, 서버로 실시간 전송되지는 않는다. 해당 영상은 발표 종료 후 VideoAnalyzer의 입력 데이터로 사용된다.

업로드된 슬라이드 파일은 발표 세션과 연결되어 Supabase Storage에 저장되며, 발표 중 슬라이드 표시 및 보고서 생성 과정에서 공통으로 사용된다.

본 모듈은 발표 시작을 위한 환경 설정을 완료하며, 사용자가 발표를 시작하면 해당 설정을 기반으로 FastAPI 서버에서 발표 세션이 생성된다.
...
인증 완료 후 Supabase에서 발급된 JWT 기반 Access Token을 사용하며, 인증이 필요한 API 요청 시 Authorization 헤더에 Bearer 토큰 형태로 포함한다.
```http
...
사용자 인증은 Supabase Authentication 기반으로 처리하며, 로그인 성공 시 Supabase을 통해 발급된 Access Token(JWT)을 반환한다.
